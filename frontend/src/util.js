...
export const constructEdge = (source_node,target_node,edge_lable) => {
  return {data:{id:uuidv4(), source: source_node,target:target_node,label: edge_lable}};
};
...
export const exportCSV = (graphEdges) => {
  const data = graphEdges.map((edge) => {
    return {
      source: edge?.data?.source,
      relation: edge?.data?.label,
